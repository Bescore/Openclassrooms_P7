<template>
	<div class="sidebar">
		...
		<div
			:key="sideindex"
			v-for="(sideuser_inactive, sideindex) in sideuser_inactive"
		>
			<img
				class="sidebar-pic"
				:src="sideuser_inactive.photo"
				alt="utilisateurs"
			/>
			<div>{{ sideuser_inactive.prenom }} {{ sideuser_inactive.nom }}</div>
			<button
				id="activate"
				@click="activeAcc(sideuser_inactive.idutilisateurs)"
			>
				Activer
			</button>
		</div>
				...
				axios
					...
					.then((response) => {
						if (response.data.active == 1) {
							...
							alert(
								`Vous avez desactivé le compte de ${response.data.prenom} !`
							);
						} else {
